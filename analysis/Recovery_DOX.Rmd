...
# Ind6REP_Spearman <- as.data.frame(fC_Matrix_Full_cpm_filter_spearmancor_dox) %>% 
...
# 
# d24_Spearman <- as.data.frame(fC_Matrix_Full_cpm_filter_spearmancor_dox) %>% 
...
# 
# Ind6_24_DOX_Spearman <- as.data.frame(fC_Matrix_Full_cpm_filter_spearmancor_dox) %>% 
...
```{r Spearman Correlations Boxplots}
#I want to make boxplots comparing the spearman correlation of different groupings of samples
...
#   ylim(0.5,1)
#   
# ####set2 - all DOX samples####
# All_DOX24_Spearman <- as.data.frame(fC_Matrix_Full_cpm_filter_spearmancor_dox) %>% 
#     dplyr::select(contains("DOX")) %>% 
#     rownames_to_column(var = "Sample")
# 
# All_DOX_Spearman_melt <- melt(All_DOX24_Spearman) %>% 
#   separate_wider_delim(., cols = Sample, names = c("Treatment", "Time", "Individual"), delim = "_", cols_remove = FALSE) %>% 
#   mutate(TxTime=paste(Treatment, Time, sep = "_")) %>% 
#   mutate(plot=paste("AllDOX")) %>% 
#   mutate(Time=factor(Time, levels = c("24", "24rec", "144rec")))
# 
# All_DOX_Spearman_plot <- 
...
#     rownames_to_column(var = "Sample")
# 
# Ind6REP_Spearman_melt <- melt(Ind6REP_Spearman) %>% 
#   separate_wider_delim(., cols = Sample, names = c("Treatment", "Time", "Individual"), delim = "_", cols_remove = FALSE) %>% 
#   mutate(TxTime=paste(Treatment, Time, sep = "_")) %>% 
#   mutate(plot=paste("Ind6REP")) %>% 
#   mutate(TxTime=factor(TxTime, levels = c("DOX_24", "DOX_24rec", "DOX_144rec", "DMSO_24", "DMSO_24rec", "DMSO_144rec")))
# 
# Ind6REP_Spearman_plot <- 
#   Ind6REP_Spearman_melt %>% 
#   ggplot(., aes(y = value, x = plot), show.legend = FALSE)+
...
#   ylim(0.5,1)
# 
# ####set 4 - all ind6REP DOX####
# Ind6REP_DOX_Spearman <- as.data.frame(fC_Matrix_Full_cpm_filter_spearmancor_dox) %>% 
#     dplyr::select(contains("Ind6REP")) %>% 
#     dplyr::select(contains("DOX")) %>% 
#     rownames_to_column(var = "Sample")
# 
# Ind6REP_DOX_Spearman_melt <- melt(Ind6REP_DOX_Spearman) %>% 
#   separate_wider_delim(., cols = Sample, names = c("Treatment", "Time", "Individual"), delim = "_", cols_remove = FALSE) %>% 
#   mutate(TxTime=paste(Treatment, Time, sep = "_")) %>% 
#   mutate(plot=paste("Ind6REP_DOX")) %>% 
#   mutate(TxTime=factor(TxTime, levels = c("DOX_24", "DOX_24rec", "DOX_144rec")))
# 
# Ind6REP_DOX_Spearman_plot <- 
#   Ind6REP_DOX_Spearman_melt %>% 
#   ggplot(., aes(y = value, x = plot), show.legend = FALSE)+
#   geom_boxplot()+
#   geom_point(aes(x = plot, y = value))+
#   theme_bw()+
...
# 
# 
# ####set 5 - compare between ind6 and ind6rep####
# Ind6_Spearman <- as.data.frame(fC_Matrix_Full_cpm_filter_spearmancor_dox) %>% 
#   dplyr::select(contains("Ind6")) %>% 
#   rownames_to_column(var = "Sample")
# 
...
#                                             "DMSO_24rec",
#                                             "DMSO_144rec"))) %>% 
#   mutate(plot=paste("Ind6 & Ind6REP"))
# 
# 
# Ind6_Spearman_plot <- 
#   Ind6_Spearman_melt %>% 
#   ggplot(., aes(y = value, x = plot), show.legend = FALSE)+
#   geom_boxplot()+
#   geom_point(aes(x = plot, y = value, colour = Individual))+
#   theme_bw()+
#   ggtitle("Ind6 + REP")+
#   xlab(" ")+
#   ylab("Spearman Correlation")+
...
# 
# 
# ####now let's look at only DOX between Ind6 and Ind6 REP for fun####
# Ind6_DOX_Spearman <- as.data.frame(fC_Matrix_Full_cpm_filter_spearmancor_dox) %>% 
#   dplyr::select(contains("Ind6")) %>% 
#   dplyr::select(contains("DOX")) %>% 
#   rownames_to_column(var = "Sample")
# 
# Ind6_DOX_Spearman_set <- Ind6_DOX_Spearman[c(31, 33, 35, 37, 39, 41),]
# 
# Ind6_DOX_Spearman_melt <- melt(Ind6_DOX_Spearman_set) %>% 
#   separate_wider_delim(., cols = Sample, names = c("Treatment", "Time", "Individual"), delim = "_", cols_remove = FALSE) %>% 
#   mutate(TxTime = paste(Treatment, Time, sep = "_")) %>% 
#   mutate(TxTime = factor(TxTime, levels = c("DOX_24", 
#                                             "DOX_24rec", 
#                                             "DOX_144rec"))) %>% 
#   mutate(plot = paste("Ind6&Ind6REP DOX"))
# 
# 
# Ind6_DOX_Spearman_plot <- 
#   Ind6_DOX_Spearman_melt %>% 
#   ggplot(., aes(y = value, x = plot), show.legend = FALSE)+
#   geom_boxplot()+
#   geom_point(aes(x = plot, y = value))+
#   theme_bw()+
#   ggtitle("Ind6 + REP DOX")+
#   xlab(" ")+
#   ylab("Spearman Correlation")+
#   ylim(0.5,1)
# 
# 
# ####now plot everything together into one plot####
# #1 All
# 
# All_Spearman_melt %>%  
#   rbind(All_DOX_Spearman_melt) %>% 
#   rbind(Ind6_Spearman_melt) %>% 
#   rbind(Ind6REP_Spearman_melt) %>% 
#   rbind(Ind6_DOX_Spearman_melt) %>% 
#   rbind(Ind6REP_DOX_Spearman_melt) %>% 
#   ggplot(., aes(x = plot, y = value))+
#   geom_boxplot()+
#   geom_point(aes(x = plot, y = value))+
...
# 
# 
# ####bring down the plots that I have above and combine####
# All_Spearman_Plots <- All_Spearman_melt %>% 
#   rbind(Ind6_Spearman_melt%>% 
#           mutate(plot="Ind6&Ind6REP"))%>% 
#   rbind(All_DOX_Spearman_melt) %>% 
#   rbind(Ind6REP_Spearman_melt) %>% 
#   rbind(d24_Spearman_melt %>% 
#           mutate(TxTime=paste(Treatment, Time, sep = "_")) %>% 
#           mutate(plot="DOX24 All Ind")) %>% 
#   rbind(Ind6_24_DOX_Spearman_melt %>% 
#           mutate(plot="Ind6&Ind6REP DOX24")) %>% 
#   rbind(Ind6REP_DOX_Spearman_melt %>% 
#           mutate(plot="Ind6REP DOX"))
# 
# 
# All_Spearman_Plots %>%  ggplot(., aes(x = plot, y = value))+
#   geom_boxplot()+
#   geom_point(aes(x = plot, y = value))+
#   theme_bw()+
#   xlab(" ")+
...
dge_d_1 <- DGEList.data.frame(counts = DOX_counts_DE_df, group = group_2d, genes = row.names(DOX_counts_DE_df))


#calculate the normalization factors with method TMM
dged_calc_1 <- calcNormFactors(dge_d, method = "TMM")
...
```{r Venn Diagram DEG Across Conditions}
#Generate a venn diagram of the DE genes across conditions
...
Cormotif_d <- dged_calc_1$counts %>% cpm(., log = TRUE)
dim(Cormotif_d)

#####EMP Note 04/24/25 - change this above matrix to my already made log2cpm matrix and convert to a df if needed####
#Cormotif_d <- fC_Matrix_Full_cpm_filter_dox #%>% 
  #rownames_to_column(var = "GeneID")

Cormotif_df_d <- as.data.frame(Cormotif_d)

#exprs.Cormotif_d <- as.matrix(Cormotif_d[,2:43])

exprs.Cormotif_d <- as.matrix(Cormotif_d)

Cormotif_df_d_cols <- colnames(Cormotif_df_d) 
Cormotif_df_d_cols_1<- Cormotif_df_d_cols[2:43]
groupid1 <- rep(c(1, 2, 3, 4, 5, 6), 7)
groupid_d <- rbind(Cormotif_df_d_cols_1, groupid1)
...
# set.seed(12345)
# new_cormotif_dox <- cormotiffit(exprs = Cormotif_d,
#                                 groupid = groupid1,
#                                 compid = compid_d,
#                                 K=1:6, 
#                                 max.iter = 500, 
...
#saveRDS(new_cormotif_dox, "data/new_cormotif_dox.RDS")

new_cormotif_dox <- readRDS("data/new_cormotif_dox.RDS")
...
#example gene - 1st gene 765 - CA6
motif3gene <- Cormotif_df_d %>% 
  rownames_to_column(var = "entrezgene_id") %>% 
  dplyr::filter(entrezgene_id == "765")
...
motif3_long <- melt(motif3gene,
                         id.vars = c("entrezgene_id"),
                         variable.name = "Sample",
                         value.name = "log2cpm")

#now add in my factors like time, tx, tx_time, and ind by breaking up the Sample column
motif3_long_df <- data.frame(tx = factor(tx_names2, levels = unique(tx_names2)),
                                  ind = factor(ind_names, levels = unique(ind_names)),
                                  txtime = factor(txtime_names2, levels = unique(txtime_names2)),
                                  time = factor(time_names2, levels = unique(time_names2)))

motif3_long_factors <- cbind(motif3_long_df, motif3_long)

motif3_long_factors %>% ggplot(aes(x = txtime, y = log2cpm))+
  geom_boxplot(aes(fill = tx))+
  geom_point(aes(color = ind)) +
  labs(title = "CA6")+
  theme_bw(base_size = 16)+
...
#first pull out the top gene from this df
#gene - LOC100288069 - 100288069
motif4gene <- Cormotif_df_d %>% 
  rownames_to_column(var = "entrezgene_id") %>% 
  dplyr::filter(entrezgene_id == "100288069")

motif4gene_long <- melt(motif4gene,
                         id.vars = c("entrezgene_id"),
                         variable.name = "Sample",
                         value.name = "log2cpm")
...
#now add in my factors like time, tx, tx_time, and ind by breaking up the Sample column
motif4_long_df <- data.frame(tx = factor(tx_names2, levels = unique(tx_names2)),
                                  ind = factor(ind_names, levels = unique(ind_names)),
                                  txtime = factor(txtime_names2, levels = unique(txtime_names2)),
                                  time = factor(time_names2, levels = unique(time_names2)))

motif4_long_factors <- cbind(motif4_long_df, motif4gene_long)

motif4_long_factors %>% ggplot(aes(x = txtime, y = log2cpm))+
  geom_boxplot(aes(fill = tx))+
  geom_point(aes(color = ind)) +
  labs(title = "LOC100288069")+
  theme_bw(base_size = 16)+
  scale_fill_manual(values = c(tx_col))+
  scale_color_manual(values = c(ind_col))+
  xlab("Conditions")+
...
```{r LogFC of All Genes Per Motif}
...
This one doesn't seem to work, most likely since it's not really picking up a motif
```{r Acute Response GO Analysis KEGG terms, include=FALSE}
...
#saveRDS(cormotifARclust_pp, "data/cormotifARclust_pp.RDS")

# cormotifARclust_pp <- readRDS("data/cormotifARclust_pp.RDS")
