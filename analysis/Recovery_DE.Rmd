...
# my_data_int <- as.integer(my_data$entrezgene_id)

# ensembl <- useEnsembl(biomart = "genes", dataset = "hsapiens_gene_ensembl")
# my_attributes <- c('entrezgene_id', 'hgnc_symbol')
# gene_list <- getBM(attributes=my_attributes,
#                    filters = "entrezgene_id",
#                    values = my_data$entrezgene_id,
#                    mart = ensembl) 
# gene_list_df <- as.data.frame(gene_list)
# AllGenes_filtered <- left_join(gene_list_df, my_data_int, by = "entrezgene_id")

#This currently isn't working how I want it to, come back to this
...
ggplot(MDM2_melt_fluo, aes(x=value, y=sample, fill = sample))+
  ggtitle("MDM2")+
    geom_bar(stat = "identity", width = 0.8)+
  theme(legend.position = "none")+
  scale_fill_manual(values = indcol_large)+
    xlab("log2cpm")+
  ylab("Conditions")


col_tx_large <- rep(c("#499FBD" ,"#DCACED", "#63666D"), 21)
indcol_large <- c(rep("#003F5C"),3, rep("#45AE91"),3, rep("#58508D"),3 , rep("#BC4099"),3, (rep("#8B3E9B"),3), (rep("#FF6361"),3), rep("#FF2362"), 3)
...
```{r Remove Unwanted Variation with RUVs}
...
counts <- as.matrix(fC_Matrix_Full_cpm_filter)
...
dge1 <- DGEList.data.frame(counts = raw_counts_mat, group = group_1, genes = row.names(raw_counts_mat))
...
fC_Matrix_Full_new %>% head()
fC_Matrix_Full_new %>% dim()

dge1 <- DGEList.data.frame(counts = matrix_cpm_new, group = group_1, genes = row.names(matrix_cpm_new))
...
toptable_Dupcor_DOX$logFC %>% hist()
toptable_Dupcor_DOXrec$logFC %>% hist()
toptable_Dupcor_DOX144$logFC %>% hist()
```
