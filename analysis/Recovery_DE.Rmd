...
counts_raw <- read_csv("C:/Users/emmap/RDirectory/Recovery_RNAseq/Recovery_RNAseq/featureCounts_Concat_Matrix_AllSamples_EMP_250220.csv")
#View(counts_raw)

counts_DE <- counts_raw %>% 
  dplyr::select(-(...1)) %>% 
  dplyr::rename(., "entrezgene_id" = "ensembl_gene_id") %>% 
  column_to_rownames(., var = "entrezgene_id")

#saveRDS(counts_DE, "data/counts_DE_raw_data.RDS")
...
#this matrix has been filtered and converted to log2cpm - this is the filtered gene list I want to make
dim(fC_Matrix_Full_cpm_filter)
#14225 genes

filtered_genes_list <- rownames(fC_Matrix_Full_cpm_filter)

length(filtered_genes_list)
#14225 genes

#filter this dataframe by the filtered gene list
counts_DE_df_1 <- counts_DE[!(rownames(counts_DE) %in% rownames(fC_Matrix_Full_cpm_filter)), ]
#14170 genes
...
dge <- DGEList.data.frame(counts = counts_DE_df_1, group = group_2, genes = row.names(counts_DE_df_1))
...
Let's use our toptables to find the top 5 DE genes in each condition
```{r Top 5 DE Genes, include = FALSE}
#plot the top 5 most DE genes for each condition
#i sorted the toptable by the top 5 most DE genes for each + found the corresponding gene name

#readRDS("data/fC_Matrix_Full_cpm_filter.RDS")
# fC_df_filter_cpm <- as.data.frame(fC_Matrix_Full_cpm_filter)
# dim(fC_df_filter_cpm)

log2cpm_DE_df <- cpm(dge_calc, log = TRUE)

dim(fC_Matrix_Full_cpm_filter)


#####24hr Tx DOX pval####
#slice the top 5 rows with the highest P value in my toptable of the given condition
top5DE_d24_p <- top.table_V.D24 %>% 
   dplyr::slice_min(., n=5, order_by=P.Value) %>% 
   rownames_to_column(var = "GeneID")
 
#make a list of the gene IDs of the top 5 DE genes
 
GeneID_top5_d24_p <- c("100996301", "100507533", "116369", "101929523", "148753")
 
#make a df of the samples (columns) that you want to look at
#d24 set
d24_gene_df <- log2cpm_DE_df %>% 
...
#top5DE_d24_log2cpm_p <- d24_gene_df %>% 
  #dplyr::filter(entrezgene_id == c("100996301", "100507533", "116369", "101929523", "148753"))

#I'll pull each of these genes out one by one and rbind
top5DE_d24_log2cpm_p_1 <- d24_gene_df %>% 
  dplyr::filter(entrezgene_id == "100996301") %>% 
  as.data.frame()

top5DE_d24_log2cpm_p_1_melt <- top5DE_d24_log2cpm_p_1 %>% 
...
#####100996301####
FOXD3 <- d24_gene_df %>% filter(entrezgene_id=="100996301")
FOXD3_new <- as.data.frame(FOXD3) %>% dplyr::select("entrezgene_id", "DOX_24_Ind1", "DMSO_24_Ind1", "DOX_24_Ind2","DMSO_24_Ind2", "DOX_24_Ind3", "DMSO_24_Ind3", "DOX_24_Ind4", "DMSO_24_Ind4", "DOX_24_Ind5", "DMSO_24_Ind5", "DOX_24_Ind6", "DMSO_24_Ind6", "DOX_24_Ind6REP", "DMSO_24_Ind6REP")
FOXD3_melt <- melt(FOXD3, variable.name = "sample")
FOXD3_melt_new <- melt(FOXD3_new, variable.name = "sample")

FOXD3_melt_df <- data.frame(tx = factor(tx_names, levels = unique(tx_names)),
...
FOXD3_melt_df2 <- data.frame(tx = factor(tx_names2, levels = unique(tx_names2)),
                               ind = factor(ind_names, levels = unique(ind_names)),
                               txtime = factor(txtime_names2, levels = unique(txtime_names2)),
                               time = factor(time_names2, levels = unique(time_names2)))
FOXD3_melt_df_all <- cbind(FOXD3_melt, FOXD3_melt_df)
FOXD3_melt_df_all2 <- cbind(FOXD3_melt_new, FOXD3_melt_df2)

FOXD3_melt <- melt(FOXD3, variable.name = "sample")
ggplot(FOXD3_melt, aes(x=value, y=sample, fill = sample))+
  ggtitle("FOXD3-AS1, top5DE DOX24 1")+
...
FOXD3_melt_df_all2 %>% ggplot(aes(x = txtime, y = value))+
...
# ggplot(top5DE_d24_log2cpm_p_1_melt, aes(x = entrezgene_id))+
...
top5DE_d24_log2cpm_p_2 <- d24_gene_df %>% 
  dplyr::filter(entrezgene_id == "100507533")

top5DE_d24_log2cpm_p_3 <- d24_gene_df %>% 
  dplyr::filter(entrezgene_id == "116369")

top5DE_d24_log2cpm_p_4 <- d24_gene_df %>% 
  dplyr::filter(entrezgene_id == "101929523")

top5DE_d24_log2cpm_p_5 <- d24_gene_df %>% 
  dplyr::filter(entrezgene_id == "148753")

top5DE_d24_log2cpm_p <- rbind(top5DE_d24_log2cpm_p_1, 
                              top5DE_d24_log2cpm_p_2, 
                              top5DE_d24_log2cpm_p_3,
                              top5DE_d24_log2cpm_p_4,
                              top5DE_d24_log2cpm_p_5)

Gene_Name_d24_p <- c("FOXD3-AS1", "SOX2-AS1", "SLC26A8", "THBS2-AS1","FAM163A")

top5DE_d24_p <- top5DE_d24_log2cpm_p %>% cbind(., Gene_Name_d24_p)

#now that I have a full dataset with these genes, I can plot them in log2cpm

top5DE_d24_p_melt <- melt(top5DE_d24_log2cpm_p, id.vars = "entrezgene_id")

#now add in the factors
ind_names_d24 <- c(rep("Ind1", 10), rep("Ind2", 10), rep("Ind3", 10), rep("Ind4", 10), rep("Ind5", 10), rep("Ind6", 10), rep("Ind6REP", 10))
time_names_d24 <- c("24")
#time_names_d24_1 <- c(rep(time_names_d24, 7))
tx_names_d24_1 <- c(rep("DOX", 5), rep("DMSO", 5))
tx_names_d24 <- c(rep(tx_names_d24_1, 7))
txtime_names2_d24_1 <- c(rep("DOX_24", 5), rep("DMSO_24", 5))
txtime_names2_d24 <- c(rep(txtime_names2_d24_1, 7))


top5DE_d24_p_melt_df <- data.frame(tx = factor(tx_names_d24, levels = unique(tx_names_d24)),
                               ind = factor(ind_names_d24, levels = unique(ind_names_d24)),
                               txtime = factor(txtime_names2_d24, levels = unique(txtime_names2_d24)),
                               time = factor(time_names_d24, levels = unique(time_names_d24)))
top5DE_d24_melt_df_all <- cbind(top5DE_d24_p_melt, top5DE_d24_p_melt_df)

top5DE_d24_melt_df_all %>% ggplot(aes(x = entrezgene_id, y = value))+
  geom_boxplot(aes(fill = tx))+
  geom_point(aes(color = ind)) +
  labs(title = "Top 5 DE Genes d24")+
  theme_bw(base_size = 16)+
  #scale_fill_manual(values = c(tx_col))+
  #scale_color_manual(values = c(ind_col))+
  xlab("Genes")+
...
#top 5 DE genes based on p-value
ggplot(top5DE_d24_p, aes(x=reorder(Gene_Name_d24_p, P.Value), y=))+
  geom_boxplot(aes(colour = P.Value))+
  ggtitle(expression("Top 5 DEG DOX 24 p value"))+
  xlab("")+
  ylim(c(-10, 10))+
...
#make a df of the samples (columns) that you want to look at
#d24 set
d24_gene_df <- fC_df_filter_cpm %>% 
  dplyr::select(matches("DOX_24")) %>% 
  dplyr::select(-(contains("rec")))
...
#I'll pull each of htese genes out one by one and cbind
top5DE_d24_log2cpm_p_1 <- d24_gene_df %>% 
  rownames_to_column(var = "entrezgene_id") %>% 
  dplyr::filter(entrezgene_id == "100996301")
...
top5DE_d24_p %>% mutate(GeneID = factor(GeneID, levels = c("100996301", "100507533", "116369", "101929523", "148753"))) %>% 
  cbind(Gene_Name_d24_p)


#####24hr Tx DOX adj pval####
top5DE_d24_a <- top.table_V.D24 %>% 
  dplyr::slice_min(., n=5, order_by=adj.P.Val) %>% 
  rownames_to_column(var = "GeneID")

Gene_Name_d24_a <- c("FOXD3-AS1", "SOX2-AS1", "SLC26A8", "FAM163A","THBS2-AS1")
 

#top 5 DE genes based on adj p value 
# ggplot(top5DE_d24_a, aes(x=reorder(Gene_Name_d24_a, adj.P.Val), y=logFC))+
#   geom_boxplot(aes(colour = adj.P.Val))+
#   ggtitle(expression("Top 5 DEG DOX 24 adj. p value"))+
#   xlab("")+
#   ylim(c(-10, 10))+
#   ylab(expression("logFC"))+
#   theme_bw()+
...
####24hr Recovery####
# top5DE_d24R_p <- top.table_V.D24r %>% 
#   dplyr::slice_min(., n=5, order_by=P.Value) %>% 
#   rownames_to_column(var = "GeneID")
# 
# Gene_Name_d24R_p <- c("LCE1B", "CSTA", "KCNK10", "OTP", "KRTAP19-1")
# 
# top5DE_d24R_p %>% mutate(GeneID = factor(GeneID, levels = c("353132", "1475", "54207", "23440", "337882"))) %>% 
#   cbind(Gene_Name_d24R_p)
#   
# top5DE_d24R_a <- top.table_V.D24r %>% 
#   dplyr::slice_min(., n=5, order_by=adj.P.Val) %>% 
#   rownames_to_column(var = "GeneID")
# 
# Gene_Name_d24R_a <- c("LCE1B", "CSTA", "KCNK10", "OTP", "KRTAP19-1")
# 
# top5DE_d24R_a %>% mutate(GeneID = factor(GeneID, levels = c("353132", "1475", "54207", "23440", "337882"))) %>% 
#   cbind(Gene_Name_d24R_a)

#top 5 DE genes based on p-value

# ggplot(top5DE_d24_p, aes(x=reorder(Gene_Name_d24R_p, P.Value), y=logFC))+
#   geom_boxplot(aes(colour = P.Value))+
#   ggtitle(expression("Top 5 DEG DOX 24R p value"))+
#   xlab("")+
#   ylim(c(-10, 10))+
#   ylab(expression("logFC"))+
#   theme_bw()+
...
# 
# #top 5 DE genes based on adj p value 
# ggplot(top5DE_d24_a, aes(x=reorder(Gene_Name_d24R_a, adj.P.Val), y=logFC))+
#   geom_boxplot(aes(colour = adj.P.Val))+
#   ggtitle(expression("Top 5 DEG DOX 24R adj. p value"))+
#   xlab("")+
#   ylim(c(-10, 10))+
#   ylab(expression("logFC"))+
#   theme_bw()+
...
# 
# ####144R DOX DE####
# top5DE_d144R_p <- top.table_V.D144r %>% 
#   dplyr::slice_min(., n=5, order_by=P.Value) %>% 
#   rownames_to_column(var = "GeneID")
# 
# Gene_Name_d144R_p <- c("LINC03025", "SLC4A10", "VIT", "PSMA8", "ADGB")
# 
# top5DE_d144R_p %>% mutate(GeneID = factor(GeneID, levels = c("440896", "57282", "5212", "143471", "79747"))) %>% 
#   cbind(Gene_Name_d144R_p)
# 
# top5DE_d144R_a <- top.table_V.D144r %>% 
#   dplyr::slice_min(., n=5, order_by=adj.P.Val) %>% 
#   rownames_to_column(var = "GeneID")
# 
# Gene_Name_d144R_a <- c("LINC03025", "SLC4A10", "VIT", "PSMA8", "ADGB")
# 
# top5DE_d144R_a %>% mutate(GeneID = factor(GeneID, levels = c("440896", "57282", "5212", "143471", "79747"))) %>% 
#   cbind(Gene_Name_d144R_a)
#   
# #top 5 DE genes based on p-value
# ggplot(top5DE_d144R_p, aes(x=reorder(Gene_Name_d144R_p, P.Value), y=logFC))+
#   geom_boxplot(aes(colour = P.Value))+
#   ggtitle(expression("Top 5 DEG DOX 144R p value"))+
#   xlab("")+
#   ylim(c(-10, 10))+
#   ylab(expression("logFC"))+
#   theme_bw()+
...
# 
# #top 5 DE genes based on adj p value 
# ggplot(top5DE_d24_a, aes(x=reorder(Gene_Name_d144R_a, adj.P.Val), y=logFC))+
#   geom_boxplot(aes(colour = adj.P.Val))+
#   ggtitle(expression("Top 5 DEG DOX 144R adj. p value"))+
#   xlab("")+
#   ylim(c(-10, 10))+
#   ylab(expression("logFC"))+
#   theme_bw()+
...
# #5FU 24hr DE genes
# #####24hr Tx DOX pval####
# top5DE_f24_p <- top.table_V.F24 %>% 
#   dplyr::slice_min(., n=5, order_by=P.Value) %>% 
#   rownames_to_column(var = "GeneID")
# 
# Gene_Name_f24_p <- c("MIR135B", "BIVM-ERCC5", "CABP4", "MIR6756", "RNA5S1")
# 
# top5DE_f24_p %>% mutate(GeneID = factor(GeneID, levels = c("442891", "100533467", "57010", "102465453", "100169751"))) %>% 
#   cbind(Gene_Name_d24_p)
#   
# 
# #####24hr Tx 5FU adj pval####
# # top5DE_f24_a <- top.table_V.F24 %>% 
# #   dplyr::slice_min(., n=5, order_by=adj.P.Val) %>% 
# #   rownames_to_column(var = "GeneID")
# 
# # Gene_Name_f24_a <- c("MIR135B", "BIVM-ERCC5", "CABP4", "MIR6756", "RNA5S1")
# # 
# # top5DE_f24_a %>% mutate(GeneID = factor(GeneID, levels = c("442891", "100533467", "57010", "102465453", "100169751"))) %>% 
# #   cbind(Gene_Name_f24_a)
# 
# #top 5 DE genes based on p-value
# ggplot(top5DE_f24_p, aes(x=reorder(Gene_Name_f24_p, P.Value), y=logFC))+
#   geom_boxplot(aes(colour = P.Value))+
#   ggtitle(expression("Top 5 DEG 5FU 24 p value"))+
#   xlab("")+
#   ylim(c(-10, 10))+
#   ylab(expression("logFC"))+
#   theme_bw()+
...
# 
# #top 5 DE genes based on adj p value 
# ggplot(top5DE_f24_p, aes(x=reorder(Gene_Name_f24_p, adj.P.Val), y=logFC))+
#   geom_boxplot(aes(colour = adj.P.Val))+
#   ggtitle(expression("Top 5 DEG 5FU 24 adj. p value"))+
#   xlab("")+
#   ylim(c(-10, 10))+
#   ylab(expression("logFC"))+
#   theme_bw()+
...
#         strip.text.y = element_text(color = "white"))
# 
# #for this one they all have an identical adj p value
# 
# #5FU 24R DE genes
# #####24hr Tx DOX pval####
# top5DE_f24R_p <- top.table_V.F24r %>% 
#   dplyr::slice_min(., n=5, order_by=P.Value) %>% 
#   rownames_to_column(var = "GeneID")
# 
# Gene_Name_f24R_p <- c("PIF1", "PKMYT1", "IQGAP3", "ESPL1","SHCBP1")
# 
# top5DE_d24_p %>% mutate(GeneID = factor(GeneID, levels = c("80119", "9088", "128239", "9700", "79801"))) %>% 
#   cbind(Gene_Name_f24R_p)
#   
# 
# #####24hr Tx DOX adj pval####
# # top5DE_f24_a <- top.table_V.F24r %>% 
# #   dplyr::slice_min(., n=5, order_by=adj.P.Val) %>% 
# #   rownames_to_column(var = "GeneID")
# 
# # Gene_Name_d24_a <- c("PIF1", "PKMYT1", "IQGAP3", "ESPL1","SHCBP1")
# # 
# # top5DE_d24_a %>% mutate(GeneID = factor(GeneID, levels = c("80119", "9088", "128239", "9700", "79801"))) %>% 
# #   cbind(Gene_Name_d24_a)
# 
# #top 5 DE genes based on p-value
# ggplot(top5DE_f24R_p, aes(x=reorder(Gene_Name_f24R_p, P.Value), y=logFC))+
#   geom_boxplot(aes(colour = P.Value))+
#   ggtitle(expression("Top 5 DEG 5FU 24R p value"))+
#   xlab("")+
#   ylim(c(-10, 10))+
#   ylab(expression("logFC"))+
#   theme_bw()+
...
# ggplot(top5DE_f24_p, aes(x=reorder(Gene_Name_f24_p, adj.P.Val), y=logFC))+
#   geom_boxplot(aes(colour = adj.P.Val))+
#   ggtitle(expression("Top 5 DEG 5FU 24R adj. p value"))+
#   xlab("")+
#   ylim(c(-10, 10))+
#   ylab(expression("logFC"))+
#   theme_bw()+
...
#         strip.text.y = element_text(color = "white"))
# 
# #5FU 144R top5DE genes
# 
# #####144hr Tx 5FU pval####
# top5DE_f144R_p <- top.table_V.F144r %>% 
#   dplyr::slice_min(., n=5, order_by=P.Value) %>% 
#   rownames_to_column(var = "GeneID")
# 
# Gene_Name_f144R_p <- c("MIR3941", "MIR6796", "SNORD115-48", "TRQ-CTG7-1","SYCE1")
# 
# top5DE_f144R_p %>% mutate(GeneID = factor(GeneID, levels = c("100500866", "102465477", "100033822", "100188998", "93426"))) %>% 
#   cbind(Gene_Name_f144R_p)
#   
# 
# #####144hr Tx 5fU adj pval####
# # top5DE_f144R_a <- top.table_V.F144r %>% 
# #   dplyr::slice_min(., n=5, order_by=adj.P.Val) %>% 
# #   rownames_to_column(var = "GeneID")
# 
# #all have the same adj p value
# # 
# # Gene_Name_f144R_a <- c("MIR3941", "MIR6796", "SNORD115-48", "TRQ-CTG7-1","SYCE1")
# # 
# # top5DE_f144R_a %>% mutate(GeneID = factor(GeneID, levels = c("100500866", "102465477", "100033822", "100188998", "93426"))) %>% 
# #   cbind(Gene_Name_f144R_a)
# 
# #top 5 DE genes based on p-value
# ggplot(top5DE_f144R_p, aes(x=reorder(Gene_Name_f144R_p, P.Value), y=logFC))+
#   geom_boxplot(aes(colour = P.Value))+
#   ggtitle(expression("Top 5 DEG 5FU 144R p value"))+
#   xlab("")+
#   ylim(c(-10, 10))+
#   ylab(expression("logFC"))+
#   theme_bw()+
...
# 
# #top 5 DE genes based on adj p value 
# ggplot(top5DE_f144R_p, aes(x=reorder(Gene_Name_f144R_p, adj.P.Val), y=logFC))+
#   geom_boxplot(aes(colour = adj.P.Val))+
#   ggtitle(expression("Top 5 DEG 5FU 144R adj. p value"))+
#   xlab("")+
#   ylim(c(-10, 10))+
#   ylab(expression("logFC"))+
#   theme_bw()+
...
```{r p value and adj p value plots of DE histograms no RUV}
#Now I'll take a look at the p value distribution as well as the adj. p value distribution across my samples
#5FU adj. p value leads me to believe nothing is significant here (which is consistent with other data)
...
top.table_V.D24 %>% ggplot(aes(x=P.Value))+
  ...
  ggtitle("DOX 24 p value distribution")

top.table_V.D24 %>% ggplot(aes(x=adj.P.Val))+
  geom_histogram(bins = 50)+
  ggtitle("DOX 24 adj p value distribution")

####5FU 24####
top.table_V.F24 %>% ggplot(aes(x=P.Value))+
  geom_histogram(bins = 50)+
  ggtitle("5FU 24 p value dist")

top.table_V.F24 %>% ggplot(aes(x=adj.P.Val))+
  geom_histogram(bins = 50)+
  ggtitle("5FU 24 adj. p value dist")

####DOX 24R####
top.table_V.D24r %>% ggplot(aes(x=P.Value))+
  geom_histogram(bins = 50)+
  ggtitle("DOX 24R p value distribution")

top.table_V.D24r %>% ggplot(aes(x=adj.P.Val))+
  geom_histogram(bins = 50)+
  ggtitle("DOX 24R adj p value distribution")

####5FU 24R####
top.table_V.F24r %>% ggplot(aes(x=P.Value))+
  geom_histogram(bins = 50)+
  ggtitle("5FU 24R p value dist")

top.table_V.F24r %>% ggplot(aes(x=adj.P.Val))+
  geom_histogram(bins = 50)+
  ggtitle("5FU 24R adj. p value dist")

####DOX 144R####
top.table_V.D144r %>% ggplot(aes(x=P.Value))+
  geom_histogram(bins = 50)+
  ggtitle("DOX 144R p value distribution")

top.table_V.D144r %>% ggplot(aes(x=adj.P.Val))+
  geom_histogram(bins = 50)+
  ggtitle("DOX 144R adj p value distribution")

####5FU 144R####
top.table_V.F144r %>% ggplot(aes(x=P.Value))+
  geom_histogram(bins = 50)+
  ggtitle("5FU 144R p value dist")

top.table_V.F144r %>% ggplot(aes(x=adj.P.Val))+
  geom_histogram(bins = 50)+
  ggtitle("5FU 144R adj. p value dist")


```

```{r pvalue and adj pvalue plots of DE with RUV geom density plots, include = FALSE}
...
initial_cormotif <- readRDS("data/initial_cormotif.RDS")
...
#plot the top DE gene for this motif based on p-value
...
```{r No Response Motif Genes}

motif_NR <- clust1

# NRmotif_genes <- gost(query = motif_NR ,
#                      organism = "hsapiens",
#                        ordered_query = FALSE,
#                        domain_scope = "custom",
...
cormotifNFclust <- gostplot(NRmotif_genes, capped = FALSE, interactive = TRUE)
cormotifNFclust

tableNR <- NRmotif_genes$result %>% 
...
tableNR_GOBP %>% ggplot(., aes(x = log_val, y = reorder(term_name, p_value), col= intersection_size)) +
...
#KEGG
tableNR_KEGG <- tableNR %>% 
  dplyr::filter(source=="KEGG") %>% 
  dplyr::select(p_value, term_name, intersection_size) %>% 
  dplyr::slice_min(., n=10, order_by=p_value) %>% 
  mutate(log_val = -log10(p_value))

tableNR_KEGG %>% ggplot(., aes(x = log_val, y = reorder(term_name, p_value), col= intersection_size)) +
  geom_point(aes(size = intersection_size)) +
...
#plot the logFC of all the genes in this motif
...
# LateDOXmotif_genes <- gost(query = motif_LateDOX ,
#                      organism = "hsapiens",
#                        ordered_query = FALSE,
#                        domain_scope = "custom",
#                        measure_underrepresentation = FALSE,
...
LDmotif_genes <- gost(query = motif_LD,
                      organism = "hsapiens",
                      ordered_query = FALSE,
                      measure_underrepresentation = FALSE,
                      evcodes = FALSE,
...
cormotifLDclust <- gostplot(LDmotif_genes, capped = FALSE, interactive = TRUE)
cormotifLDclust

tableLD <- LDmotif_genes$result %>% 
  dplyr::select(c(source, term_id, term_name, intersection_size, term_size, p_value))
...
tableLD_GOBP %>% ggplot(., aes(x = log_val, y = reorder(term_name, p_value), col= intersection_size)) +
  geom_point(aes(size = intersection_size)) +
  ggtitle("Late DOX Response Enriched GO:BP Terms")+
  xlab(expression("-log"[10]~"p-value"))+
  guides(col="none", size= guide_legend(title = "# of intersected \n terms"))+
  ylab("GO:BP term")+
  scale_y_discrete(labels = scales::label_wrap(30))+
...
#KEGG
tableLD_KEGG <- tableLD %>% 
  dplyr::filter(source=="KEGG") %>% 
  dplyr::select(p_value, term_name, intersection_size) %>% 
  dplyr::slice_min(., n=10, order_by=p_value) %>% 
  mutate(log_val = -log10(p_value))

tableLD_KEGG %>% ggplot(., aes(x = log_val, y = reorder(term_name, p_value), col= intersection_size)) +
  geom_point(aes(size = intersection_size)) +
  ggtitle("Late DOX Response Enriched KEGG Terms")+
  xlab(expression("-log"[10]~"p-value"))+
  guides(col="none", size= guide_legend(title = "# of intersected \n terms"))+
  ylab("KEGG term")+
  scale_y_discrete(labels = scales::label_wrap(30))+
...
```{r Early DOX Response Motif Genes}
...
EDmotif_genes <- gost(query = motif_ED,
                      organism = "hsapiens",
                      ordered_query = FALSE,
                      measure_underrepresentation = FALSE,
                      evcodes = FALSE,
...
cormotifEDclust <- gostplot(EDmotif_genes, capped = FALSE, interactive = TRUE)
cormotifEDclust

tableED <- EDmotif_genes$result %>% 
  dplyr::select(c(source, term_id, term_name, intersection_size, term_size, p_value))
...
tableED_GOBP %>% ggplot(., aes(x = log_val, y = reorder(term_name, p_value), col= intersection_size)) +
  geom_point(aes(size = intersection_size)) +
  ggtitle("Early DOX Response Enriched GO:BP Terms")+
  xlab(expression("-log"[10]~"p-value"))+
  guides(col="none", size= guide_legend(title = "# of intersected \n terms"))+
  ylab("GO:BP term")+
  scale_y_discrete(labels = scales::label_wrap(30))+
...
#KEGG
tableED_KEGG <- tableED %>% 
  dplyr::filter(source=="KEGG") %>% 
  dplyr::select(p_value, term_name, intersection_size) %>% 
  dplyr::slice_min(., n=10, order_by=p_value) %>% 
  mutate(log_val = -log10(p_value))

tableED_KEGG %>% ggplot(., aes(x = log_val, y = reorder(term_name, p_value), col= intersection_size)) +
  geom_point(aes(size = intersection_size)) +
  ggtitle("Early DOX Response Enriched KEGG Terms")+
  xlab(expression("-log"[10]~"p-value"))+
  guides(col="none", size= guide_legend(title = "# of intersected \n terms"))+
  ylab("KEGG term")+
  scale_y_discrete(labels = scales::label_wrap(30))+
