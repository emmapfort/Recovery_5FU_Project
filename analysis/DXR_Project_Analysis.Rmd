...
# Load DEGs Data
DOX_24T_1 <- read.csv("data/new/DEGs/Toptable_RUV_24T.csv")
DOX_24R_1 <- read.csv("data/new/DEGs/Toptable_RUV_24R.csv")
DOX_144R_1 <- read.csv("data/new/DEGs/Toptable_RUV_144R.csv")
...
```{r p53 Target Genes logFC HM, fig.height=50, fig.width=10}
# Load libraries
...
#Filter the data based on each motif
filt_toptable_dxr_fin_unassign <- combined_toptables_dxr %>% 
...
filt_toptable_abs_fin_unassign <- combined_toptables_dxr %>% 
  dplyr::filter(Entrez_ID  %in% unassign_genes_fin) %>% 
  mutate(absFC = abs(logFC)) %>% 
  mutate(time = factor(Time, levels = c("24", "24R", "144R"), labels = c("24hr", "24hr Recovery", "144hr Recovery"))) %>% 
  ggplot(., aes(x = time, y = absFC))+
...
####GO KEGG####
unassigned_genes_matrix <- as.matrix(unassign_genes_fin)
...
# Entrez_ID_tmm_RUV <- RUV_counts_tmm$Entrez_ID
...
motif.fitted_ruv <- readRDS("data/new/motif.fitted_ruv_250624.RDS")
```
...
```{r RUVs Cormotif no TMM}
#now I want to try this without doing the TMM normalization, as it's likely unnecessary
#the normalized counts are based on unwanted variation - residuals
#they are not TMM normalized or anything like that

RUV_CMF_normcounts_rep <- as.data.frame(set1$normalizedCounts)

#first want to cut out the replicate samples as not needed for Cormotif
RUV_CMF_normcounts <- RUV_CMF_normcounts_rep %>% 
  dplyr::select(-(contains("6R")))

#then create a dge object of the normalized counts from RUVs
dge_RUV_CMF <- DGEList(RUV_CMF_normcounts)
dim(dge_RUV_CMF)

#to put into Cormotif must be in log2cpm
dge_RUV_CMF_cpm <- cpm(dge_RUV_CMF, log = TRUE)
dim(dge_RUV_CMF_cpm)

#now that the normalized counts are in cpm format, format my matrix for Cormotif

cmf_RUV_df <- dge_RUV_CMF_cpm %>% 
  as.data.frame() %>% 
  rownames_to_column(., var = "Entrez_ID")

#write.csv(x = dge_RUV_CMF_cpm, file = "data/new/RUV/cormotif_normcountsRUVs_matrix.csv")

#reorder my test matrix to match the new groupid I've made
#I want my columns to be in this order:
#DOX24T 1-6, DOX24R 1-6, DOX144R 1-6, DMSO24T 1-6, DMSO24R 1-6, DMSO144R 1-6
CMF_RUV <- read.csv("data/new/RUV/cormotif_normcountsRUVs_matrix.csv")
dim(CMF_RUV)
#14319 genes across 37 cols (1 is Entrez_ID)
CMF_RUV_df <- data.frame(CMF_RUV)

rownames(CMF_RUV_df) <- CMF_RUV_df$Entrez_ID
exprs.CMF_RUV <- as.matrix(CMF_RUV_df[,2:37])
dim(exprs.cormotif)
#14319 genes in 36 columns, set up for plugging into Cormotif
...
saveRDS(motif.fit_RUV, "data/new/RUV/motif.fit_RUVnormcounts_250625.RDS")
motif.fit_ruv <- readRDS("data/new/motif.fit_RUVnormcounts_250625.RDS")


```

```{r RUVs Plot Cormotif}
...
lognormcounts_CMF_rep <- as.data.frame(set1$normalizedCounts)
lognormcounts_CMF_rep <- log2(set1$normalizedCounts + 1)
#add a pseudocount to ensure any values are not negative or zero

#first want to cut out the replicate samples as not needed for Cormotif
lognormcounts_CMF <- as.data.frame(lognormcounts_CMF_rep) %>% 
  dplyr::select(-(contains("6R")))

# #then create a dge object of the normalized counts from RUVs
# dge_logNC_RUV <- DGEList(lognormcounts_CMF)
# dim(dge_RUV_CMF)
# #14319 genes in 36 columns as anticipated

#now go ahead and start putting together your data for Cormotif
logNC_CMF_RUV <- lognormcounts_CMF %>% 
  as.data.frame() %>% 
  rownames_to_column(., var = "Entrez_ID")
...
#I want my columns to be in this order:
#DOX24T 1-6, DOX24R 1-6, DOX144R 1-6, DMSO24T 1-6, DMSO24R 1-6, DMSO144R 1-6
logNC_CMF_RUV <- read.csv("data/new/RUV/Cormotif_log2normcounts_RUV_matrix.csv")
dim(logNC_CMF_RUV)
#14319 genes across 37 cols (1 is Entrez_ID)
logNC_CMF_RUV_df <- data.frame(logNC_CMF_RUV)

rownames(logNC_CMF_RUV_df) <- logNC_CMF_RUV_df$Entrez_ID
exprs.logNC_CMF_RUV <- as.matrix(logNC_CMF_RUV_df[,2:37])
dim(exprs.logNC_CMF_RUV)
#14319 genes in 36 columns, set up for plugging into Cormotif
