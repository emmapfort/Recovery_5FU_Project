...
# groupid_1 <- groupid_csv %>% as.vector()
...
#now before I change any of these to be like the shade of the box, just do 0.5
...
) + ggtitle("Cormotif Specific and Shared DEGs (>0.6)")+
  theme(
    plot.title = element_text(size = 16, face = "bold"),  # Increase title size
    text = element_text(size = 16)  # Increase text size globally
  )
...
```{r Extract Gene Probabilities by Cluster}
...
```{r Cormotif Gene Pie Chart}
clusterdata_dxr <- data.frame(
  Category = c("Motif 1", "Motif 2", "Motif 3"), 
  Value = c(length(prob_all_1), length(prob_all_2), length(prob_all_3))
)
...
```{r Plot Cormotif logFC Conditions}
#Extract the gene IDs from each motif

##motif 1
motif1_genes <- prob_all_1
length(motif1_genes)
...
#Filter the data based on each motif
filt_toptable_dxr <- combined_toptables_dxr %>% 
  dplyr::filter(entrezgene_ID  %in% clust1) %>% 
  mutate(absFC = abs(logFC)) %>% 
  mutate(time = factor(Time, levels = c("24", "24R", "144R"), labels = c("24hr", "24hr Recovery", "144hr Recovery"))) %>% 
  ggplot(., aes(x = time, y = logFC))+
  geom_boxplot(aes(fill = time))+
...
filt_toptable_2_dxr <- combined_toptables_dxr %>% 
  dplyr::filter(entrezgene_ID  %in% clust2) %>% 
  mutate(absFC = abs(logFC)) %>% 
  mutate(time = factor(Time, levels = c("24", "24R", "144R"), labels = c("24hr", "24hr Recovery", "144hr Recovery"))) %>% 
  ggplot(., aes(x = time, y = logFC))+
  geom_boxplot(aes(fill = time))+
...
#motif 3
filt_toptable_3_dxr <- combined_toptables_dxr %>% 
  dplyr::filter(entrezgene_ID  %in% clust3) %>% 
  mutate(absFC = abs(logFC)) %>% 
  mutate(time = factor(Time, levels = c("24", "24R", "144R"), labels = c("24hr", "24hr Recovery", "144hr Recovery"))) %>% 
  ggplot(., aes(x = time, y = logFC))+
  geom_boxplot(aes(fill = time))+
...
#now plot the abs logFC for each of these too
filt_toptable_abs <- combined_toptables_dxr %>% 
  dplyr::filter(entrezgene_ID  %in% clust1) %>% 
  mutate(absFC = abs(logFC)) %>% 
  mutate(time = factor(Time, levels = c("24", "24R", "144R"), labels = c("24hr", "24hr Recovery", "144hr Recovery"))) %>% 
  ggplot(., aes(x = time, y = absFC))+
...
#motif 2
filt_toptable_2_abs <- combined_toptables_dxr %>% 
  dplyr::filter(entrezgene_ID  %in% clust2) %>% 
  mutate(absFC = abs(logFC)) %>% 
  mutate(time = factor(Time, levels = c("24", "24R", "144R"), labels = c("24hr", "24hr Recovery", "144hr Recovery"))) %>% 
  ggplot(., aes(x = time, y = absFC))+
...
#motif 2
filt_toptable_3_abs <- combined_toptables_dxr %>% 
  dplyr::filter(entrezgene_ID  %in% clust3) %>% 
  mutate(absFC = abs(logFC)) %>% 
  mutate(time = factor(Time, levels = c("24", "24R", "144R"), labels = c("24hr", "24hr Recovery", "144hr Recovery"))) %>% 
  ggplot(., aes(x = time, y = absFC))+
...
motif1_genes <- boxplots_cormotif[boxplots_cormotif$Entrez_ID %in% prob_all_1,]
dim(motif1_genes)
#6353 genes in 44 cols
motif2_genes <- boxplots_cormotif[boxplots_cormotif$Entrez_ID %in% prob_all_2,]
dim(motif2_genes)
#6353 genes in 44 cols
motif3_genes <- boxplots_cormotif[boxplots_cormotif$Entrez_ID %in% prob_all_3,]
dim(motif3_genes)
#461 genes in 44 cols

#pull out some random genes for each using sample

m1_genes <- motif1_genes[sample(nrow(motif1_genes), 3), , drop = FALSE]

#Define gene list(s)
#these are the genes pulled out by m1_genes
initial_test_genes1 <- c("TRIM66")  
#Add more gene symbols as needed or add more categories

#Now put in the function I want to use to generate boxplots of genes
process_gene_datam1 <- function(gene) {
  gene_data <- motif1_genes %>% filter(SYMBOL == gene)
  long_data <- gene_data %>%
    pivot_longer(cols = -c(Entrez_ID, SYMBOL), names_to = "Sample", values_to = "log2CPM") %>%
    mutate(
      Drug = case_when(
...
#Generate Boxplots from the above function using our gene list above
for (gene in initial_test_genes1) {
  gene_data <- process_gene_datam1(gene)
  p <- ggplot(gene_data, aes(x = Condition, y = log2CPM, fill = Drug)) +
    geom_boxplot(outlier.shape = NA) +
    geom_point(aes(color = Indv), size = 2, alpha = 0.5, position = position_jitter(width = -1, height = 0)) +
    scale_fill_manual(values = c("DOX" = "#499FBD", "DMSO" = "#BBBBBC")) +
...
m2_genes <- motif2_genes[sample(nrow(motif2_genes), 3), , drop = FALSE]

#Define gene list(s)
#these are the genes pulled out by m1_genes
initial_test_genes2 <- c("DUSP8")  
#Add more gene symbols as needed or add more categories

#Now put in the function I want to use to generate boxplots of genes
process_gene_datam2 <- function(gene) {
  gene_data <- motif2_genes %>% filter(SYMBOL == gene)
  long_data <- gene_data %>%
    pivot_longer(cols = -c(Entrez_ID, SYMBOL), names_to = "Sample", values_to = "log2CPM") %>%
    mutate(
      Drug = case_when(
...
#Generate Boxplots from the above function using our gene list above
for (gene in initial_test_genes2) {
  gene_data <- process_gene_datam2(gene)
  p <- ggplot(gene_data, aes(x = Condition, y = log2CPM, fill = Drug)) +
    geom_boxplot(outlier.shape = NA) +
    geom_point(aes(color = Indv), size = 2, alpha = 0.5, position = position_jitter(width = -1, height = 0)) +
    scale_fill_manual(values = c("DOX" = "#499FBD", "DMSO" = "#BBBBBC")) +
...
m3_genes <- motif3_genes[sample(nrow(motif3_genes), 3), , drop = FALSE]

#Define gene list(s)
#these are the genes pulled out by m1_genes
initial_test_genes3 <- c("PNMA8B")  
#Add more gene symbols as needed or add more categories

#Now put in the function I want to use to generate boxplots of genes
process_gene_datam3 <- function(gene) {
  gene_data <- motif3_genes %>% filter(SYMBOL == gene)
  long_data <- gene_data %>%
    pivot_longer(cols = -c(Entrez_ID, SYMBOL), names_to = "Sample", values_to = "log2CPM") %>%
    mutate(
      Drug = case_when(
...
#Generate Boxplots from the above function using our gene list above
for (gene in initial_test_genes3) {
  gene_data <- process_gene_datam3(gene)
  p <- ggplot(gene_data, aes(x = Condition, y = log2CPM, fill = Drug)) +
    geom_boxplot(outlier.shape = NA) +
    geom_point(aes(color = Indv), size = 2, alpha = 0.5, position = position_jitter(width = -1, height = 0)) +
    scale_fill_manual(values = c("DOX" = "#499FBD", "DMSO" = "#BBBBBC")) +
...
motif1_genes_clust <- boxplots_cormotif[boxplots_cormotif$Entrez_ID %in% clust1_p,]
dim(motif1_genes_clust)
#6827 genes in 44 cols
motif2_genes_clust <- boxplots_cormotif[boxplots_cormotif$Entrez_ID %in% clust2_p,]
dim(motif2_genes_clust)
#6474 genes in 44 cols
motif3_genes_clust <- boxplots_cormotif[boxplots_cormotif$Entrez_ID %in% clust3_p,]
dim(motif3_genes_clust)
#607 genes in 44 cols
...
#Define gene list(s)
#these are the genes pulled out by m1_genes
initial_test_genes1_clust <- c(m1_genes$SYMBOL)  
#Add more gene symbols as needed or add more categories
...
#Generate Boxplots from the above function using our gene list above
for (gene in initial_test_genes1_clust) {
  gene_data <- process_gene_datam1(gene)
  p <- ggplot(gene_data, aes(x = Condition, y = log2CPM, fill = Drug)) +
    geom_boxplot(outlier.shape = NA) +
    geom_point(aes(color = Indv), size = 2, alpha = 0.5, position = position_jitter(width = -1, height = 0)) +
...
m2_genes_clust <- motif2_genes_clust[sample(nrow(motif2_genes_clust), 3), , drop = FALSE]
...
m3_genes_clust <- motif3_genes_clust[sample(nrow(motif3_genes_clust), 3), , drop = FALSE]

#Define gene list(s)
#these are the genes pulled out by m1_genes
initial_test_genes3_clust <- c(m3_genes_clust$SYMBOL)  
#Add more gene symbols as needed or add more categories

#Now put in the function I want to use to generate boxplots of genes
process_gene_datam3_clust <- function(gene) {
  gene_data <- motif3_genes_clust %>% filter(SYMBOL == gene)
  long_data <- gene_data %>%
    pivot_longer(cols = -c(Entrez_ID, SYMBOL), names_to = "Sample", values_to = "log2CPM") %>%
    mutate(
      Drug = case_when(
...
#Generate Boxplots from the above function using our gene list above
for (gene in initial_test_genes3_clust) {
  gene_data <- process_gene_datam3_clust(gene)
  p <- ggplot(gene_data, aes(x = Condition, y = log2CPM, fill = Drug)) +
    geom_boxplot(outlier.shape = NA) +
    geom_point(aes(color = Indv), size = 2, alpha = 0.5, position = position_jitter(width = -1, height = 0)) +
    scale_fill_manual(values = c("DOX" = "#499FBD", "DMSO" = "#BBBBBC")) +
