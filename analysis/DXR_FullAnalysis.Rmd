...
```{r Create Dataframe From Counts}
...
counts_raw_matrix <- counts_raw_df %>% column_to_rownames(var = "entrezgene_id") %>% as.matrix()
...
#make a list to filter my counts by from this list
...
#now use cpm to convert this to log2cpm
filt_cpm_dxr <- cpm(filt_counts_dxr, log = TRUE)


#I need to remove the Ind6REP individual before using this for limma and Cormotif

filt_counts_norep <- filt_counts_dxr %>%
  as.data.frame() %>% 
  dplyr::select(-(contains("Ind6REP")))

dim(filt_counts_norep)
#14319 genes
...
dge_filt <- DGEList(counts = filt_counts_norep, group = group_fac, genes = row.names(filt_counts_norep), )

View(dge_filt)

dge_filt_tmm <- calcNormFactors(dge_filt, method = "TMM")

View(dge_filt_tmm$samples)
#you can see that the normalization factors for library size have been calculated using TMM methods

#now we use cpm to use these TMM normalized library sizes for log2cpm
dge_filt_tmm_cpm <- cpm(dge_filt_tmm, log = TRUE)
dim(dge_filt_tmm_cpm)
View(dge_filt_tmm_cpm)
class(dge_filt_tmm_cpm)

dge_filt_tmm_cpm_1 <- dge_filt_tmm_cpm %>% 
  as.data.frame() %>%
  rownames_to_column(var = "entrezgene_ID")
dim(dge_filt_tmm_cpm_1)

#now I can make this into the expression matrix that is needed for cormotif
exprs_tmm_dxr <- as.matrix(dge_filt_tmm_cpm_1[,2:37])


```

```{r Cormotif Function, warning=FALSE, include=FALSE}
...
```{r Cormotif Data}
#Now put together your data for Cormotif

Cormotif_new <- cpm(dge_filt_tmm, log = TRUE)
class(Cormotif_new)

Cormotif_df_new <- as.data.frame(Cormotif_new)
dim(Cormotif_df_new)

groupid_new <- rep(c(1, 2, 3, 4, 5, 6), 6)
#Sayan tx timepoint conc
#mine will then be drug time ind

compid_new <- data.frame(c1 = c(1, 3, 5), c2 = c(2, 4, 6))
```

```{r Run Cormotif}
#set.seed(1919191)
Cormotif_new_plot <- cormotiffit(exprs = Cormotif_new,
                                 groupid = groupid_new,
                                 compid = compid_new,
                                 K=1:8,
                                 max.iter = 500)
...
```{r Plot AIC BIC Cormotif}

plotIC(Cormotif_new_plot)

plotMotif(Cormotif_new_plot)
