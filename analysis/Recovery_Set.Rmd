...
#saveRDS(dged_calc, "data/dge_calc_matrix.RDS")
...
Cormotif_matrix <- readRDS("data/dge_calc_matrix.RDS")
Cormotif_tmm_counts <- Cormotif_matrix$counts

annot_d <- data.frame("samples" = colnames(Cormotif_tmm_counts)) %>% separate_wider_delim(., cols = samples, names = c("Tx", "Time", "Ind"), delim = "_", cols_remove = FALSE) %>% unite(., col = "Tx_Time", Tx, Time, sep = "_", remove = FALSE) 

Cormotif_dox <- cpm(Cormotif_tmm_counts, log = TRUE)

Cormotif_df_d <- as.data.frame(Cormotif_dox)
...
set.seed(12345)
Cormotif_dox_tmm <- cormotiffit(exprs = Cormotif_dox,
                                groupid = groupid_d,
                                compid = compid_d,
                                norm.factor.method = "TMM",
                                BIC = TRUE,
                                K=1:8,
                                max.iter = 500,
                                runtype = "logCPM")
...
Cormotif_dox <- readRDS("data/final_data/Cormotif_TMM_dox.RDS")

```

```{r Cormotif AIC + BIC Plot}

plotIC(Cormotif_dox_tmm)

```

```{r Cormotif Plot Motifs}

plotMotif(Cormotif_dox_tmm)
...
readRDS("data/final_data/Cormotif_prob_gene_list_dox.RDS")
