...
#Have to do a log2cpm conversion here
...
# 
# #timepoint colors
# time_col_df <- list(Timepoint = c("24" = "#046A38", "24rec" = "#0050B5", "144rec" = "#B725AD"))
...
dge <- DGEList.data.frame(counts = counts_DE_filter1, group = group_1, genes = row.names(counts_DE_filter))
...
controls <- rownames(reps)
differences <- matrix(data = c(1,10, 2, 11, 3 ,12, 4, 13, 5 ,14, 6, 15, 7, 16, 8, 17, 9, 18), byrow = TRUE, nrow = 9)
seqRUVs <- RUVs(fC_Matrix_cpm_filter_ruv, controls, k=2, differences, isLog = TRUE)
pData(seqRUVs)
#head(seqRUVs)
